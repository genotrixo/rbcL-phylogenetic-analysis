...
# 4. Circular tree visualization (color mapped to branch length)
p_circular_gradient <- ggtree(my_tree, layout = "circular", size = 1.1, aes(color = branch.length)) + 
  
  # Sqrt transformation to enhance visibility of minor branch length variations
  scale_color_gradientn(
    name = "Evolutionary Distance", 
    colors = c("#000080", "#00BFFF", "#32CD32", "#FFD700", "#FF0000"), 
    trans = "sqrt" 
  ) +
  ...
  # --- Highlight target species with asterisks ---
  geom_tippoint(
...
# Export high-resolution plot for publication
ggsave(
  filename = "My_High_Quality_Plot.png", # Output filename
  plot = p_circular_gradient,            # Plot object
  width = 15,                            # Width
  height = 12,                           # Height
  units = "in",                          # Dimensions in inches
  dpi = 600,                             # 600 DPI for publication quality
